...
def savefig( fn,vdi):
    ...
    plt.title('VDI={}'.format(vdi))
    plt.savefig('VDI_distri_%d' % fig_num)
    return fn
...
class Agent:
    ...
    def csact(self, input_state) -> list:
        action_move = []
        input = torch.unsqueeze(torch.FloatTensor(input_state), 0)
        action_prob = torch.FloatTensor(self.target_net(input))
        for i in range(int(self.action_space / 5)):
            dist = Categorical(action_prob[i * 5:i * 5 + 5])
            idx = dist.sample()  # 返回0~4的tensor
            idx_list = idx.tolist()
            action_move.append((idx_list + i * 5))
    ...
    def step(self, a, cap_idx, dc_val):
        ...
        for i in range(len(a)):
            action[i], cap[i] = move(a[i], cap_idx[i])
        str_dc = ''
    ...
    def update(self):
        ...
        batch_a = torch.LongTensor(batch_memory[:, nstate:nstate + 10].astype(int))
        batch_r = torch.FloatTensor(batch_memory[:, nstate + 10:nstate + 11])
        batch_s_ = torch.FloatTensor(batch_memory[:, -nstate:])

        # MSELoss
        q_eval = self.eval_net(batch_s).gather(1, batch_a)  # shape = (batch, 10)
        q_next = self.target_net(batch_s_).detach()
        q_next_max = torch.zeros(10)
        for i in range(10):
            q_next_max[i] = max(q_next[i * 5:i * 5 + 5])
...
for eps in range(episodes):
    if eps == 0:
        dnn.reset()
        fig_num = savefig(fig_num,'initial')
    if eps % 500 == 0:
    ...
    a = dnn.csact(s)
    s_, a, cap = dnn.step(a, cap_idx, 1e-9)
    r = (target_vdi - np.sum(s_)) / target_vdi
    dnn.store_transition(s,a,r,s_)
    reward.append(r)
    total_vdis.append(np.sum(s_))
    s = s_
    if dnn.mem_cnt > 0.8 * memorysize:
        dnn.update()
    if r > 0:
        fig_num = savefig(fig_num,np.sum(s_))
